...
static void msn_send_voice_clip(MsnSwitchBoard *swboard, const char *filepath);
...
static void
voice_clip_cb(PurpleBlistNode *node, gpointer data)
{
    PurpleBuddy *buddy;
    PurpleConnection *gc;
    ...
    if ((swboard = msn_switchboard_new(session)) != NULL)
    {
        purple_request_file(swboard, _("Send Voice Clip"),
                            NULL, FALSE, G_CALLBACK(msn_send_voice_clip), NULL,
                            NULL, NULL, NULL,
                            swboard);
    }
}
...

        {
            ...
            if (user)
            {
                ...
                if (!pecan_contact_is_account (user))
                {
                    ...
                    act = purple_menu_action_new (_("Send Voice Clip"),
                                                  PURPLE_CALLBACK (voice_clip_cb),
                                                  NULL, NULL);
                    m = g_list_append(m, act);
                }
            }
            ...
            PurpleStatus *status)
{
    ...
    if (gc)
    {
        ...
        if (purple_account_get_bool_without_default_value (account, "use_psm"))
            pecan_update_personal_message (session);
    }
}
