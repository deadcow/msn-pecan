...
msn_session_new(PurpleAccount *account)
{
	...
	session = g_new0(MsnSession, 1);
        ...
        session->http_method = purple_account_get_bool_without_default_value (account, "http_method");
        session->server_alias = purple_account_get_bool_without_default_value (account, "server_alias");
        session->use_directconn = purple_account_get_bool_without_default_value (account, "use_directconn");
        session->use_userdisplay = purple_account_get_bool_without_default_value (account, "use_userdisplay");
	...
	/* The following code will remove the "message" attr from account statuses. */
	if (!purple_account_get_bool_without_default_value (session->account, "use_psm"))
	{
		GList *types, *statuses;
		PurpleStatusType *old_type, *type;
		...
		for (types = account->status_types; types != NULL; types = types->next)
		{
			if (purple_status_type_get_attr (types->data, "message"))
			{
				primitive = purple_status_type_get_primitive(types->data);
				id = purple_status_type_get_id(types->data);
				name = purple_status_type_get_name(types->data);
				saveable = purple_status_type_is_saveable(types->data);
				user_settable = purple_status_type_is_user_settable(types->data);
				independent = purple_status_type_is_independent(types->data);

				types->data = purple_status_type_new_full (primitive, id, name, saveable, user_settable, independent);
			}
		}

		for (statuses = purple_presence_get_statuses (account->presence); statuses != NULL; statuses = statuses->next)
		{
			old_type = purple_status_get_type (statuses->data);
			if (purple_status_type_get_attr (old_type, "message"))
			{
				type = (PurpleStatusType*) purple_status_type_find_with_id(types, purple_status_type_get_id (old_type));
				if (type)
				{
					purple_status_destroy (statuses->data);
					statuses->data = purple_status_new(type, account->presence);
				}
				else
				{
					types = account->status_types;
					while (purple_status_type_get_primitive(old_type) != purple_status_type_get_primitive(types->data))
						types = types->next;
					purple_status_destroy (statuses->data);
					statuses->data = purple_status_new(types->data, account->presence);
				}
			}
		}
	}
